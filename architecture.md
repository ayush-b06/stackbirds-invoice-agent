...
Uses Claude (claude-sonnet-4) via the vision API (PDF or image as base64). A strict schema prompt instructs the model to return only JSON, set `confidence` 0–1, and populate `extraction_warnings` for any ambiguous fields rather than silently hallucinating values.
